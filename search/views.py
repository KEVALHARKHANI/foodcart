...
class Search_bar(ListAPIView):
    model = food
    queryset = food.objects.all()
    #template_name = 'search_result.html'
    serializer_class = Foodserializers
    filter_backends = (SearchFilter)
    search_fields=('foodname','restroname')


    #def get_queryset(self, **kwargs):
    #    context=super().get_context_data(**kwargs)
     #   context['filter']=Searchserializers(self.request.GET,queryset=self.get_queryset())
      #  return context
...
def search_bar(request):
    ...
    if food_queryset_list or restro_queryset_list:
        return render(request,'search_result.html',{'restro':set(restro_queryset_list),'food':set(food_queryset_list),'query_error':query_error,'query':query})
    else:
